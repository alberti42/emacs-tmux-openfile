...
## Requires release-notes/release-TAG.md to exist and be non-empty.
version-bump:
	...
	@[ -s "release-notes/release-$(TAG).md" ] \
		|| { echo "error: release-notes/release-$(TAG).md is missing or empty; write it first"; exit 1; }
	@sed -i '' "s/^;; Version:.*$$/;; Version: $$(echo $(TAG) | sed 's/^v//')/" tmux-tandem.el
	@git add tmux-tandem.el "release-notes/release-$(TAG).md"
	@git diff --cached --quiet || git commit -m "chore(release): bump to $(TAG)"
	@git tag "$(TAG)"
	@echo "Tagged $(TAG). Run: make push-release-draft"
